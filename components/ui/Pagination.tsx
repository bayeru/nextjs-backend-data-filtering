...
const Pagination = ({ page, totalPages }: PaginationProps) => {
	...
	return (
		<nav className="flex mx-auto mt-8 text-slate-500">
			<Link
				href={`?page=${page - 1}`}
				className="inline-flex px-3 py-2 justify-center items-center mr-2 bg-white shadow-sm border border-slate-200 rounded"
			>
			...
			<Link
				href={`?page=${page + 1}`}
				className="inline-flex px-3 py-2 justify-center items-center ml-2 bg-white shadow-sm border border-slate-200 rounded"
			>
				<svg className="w-4 h-4 fill-slate-500" viewBox="0 0 16 16">
